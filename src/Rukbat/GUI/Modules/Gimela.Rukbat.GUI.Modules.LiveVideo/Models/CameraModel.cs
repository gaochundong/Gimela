...
namespace Gimela.Rukbat.GUI.Modules.LiveVideo.Models
{
  public class CameraModel : ModelBase
  {
    ...
    public void UnpublishCamera(Camera camera, string address, int port)
    {
      try
      {
        AsyncWorkerHandle<Camera> handle = AsyncWorkerHelper.DoWork<Camera>(
          delegate(object sender, DoWorkEventArgs e)
          {
            e.Result = UnpublishCameraSync(camera, address, port);
          },
          null, null);
      }
      catch (Exception ex)
