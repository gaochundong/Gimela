...
using Gimela.Data.Mapping.Internal;
using System.Linq;
...
namespace Gimela.Data.Mapping
{
    internal class MappingExpression : IMappingExpression, IMemberConfigurationExpression
    {
        ...
        public void MapFrom(string sourceMember)
        {
            ...
            if (!members.Any()) throw new MapperConfigurationException(string.Format("Unable to find source member {0} on type {1}", sourceMember, _typeMap.SourceType.FullName));
            if (members.Skip(1).Any()) throw new MapperConfigurationException(string.Format("Source member {0} is ambiguous on type {1}", sourceMember, _typeMap.SourceType.FullName));
