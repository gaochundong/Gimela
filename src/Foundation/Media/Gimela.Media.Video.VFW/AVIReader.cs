...
namespace Gimela.Media.Video.VFW
{
  /// <summary>
    ...
    public Bitmap GetNextFrame()
    {
      lock (this)
      {
        ...
        if (bitmapInfoHeader.height > 0)
        {
          ...
          {
            Win32.memcpy(new IntPtr(dst), new IntPtr(src), (UIntPtr)srcStride);
            dst -= dstStride;
            src += srcStride;
          }
        }
        else
        {
          ...
          Win32.memcpy(new IntPtr(dst), new IntPtr(src), (UIntPtr)(srcStride * height));
        }
